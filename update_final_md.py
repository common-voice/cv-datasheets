...
def get_localize_section(lang_code: str, section: str) -> str:
    i8n = {
        "es": {"gender": "Género", "age": "Edad", "sample": "Muestra", "community_links": "Enlaces comunitarios", "license": "Licencia", "fields": "Campos", "get_involved": "¡Involúcrate!"},
        "en": {"gender": "Gender", "age": "Age", "sample": "Sample", "community_links": "Community links", "license": "License", "fields": "Fields", "get_involved": "Get involved!"},
    }
    return i8n[lang_code][section]

def replace_content_section(draft_ds: CVDatasheet, final_ds: CVDatasheet, section_title: str, lang_code: str):
...
def append_content_section(draft_ds: CVDatasheet, final_ds: CVDatasheet, section_title: str, lang_code: str):
    section_title = get_localize_section(lang_code, section_title)
    draft_section = draft_ds._section_map.get(section_title, None)
    final_ds.append_content(section_title, draft_section.content)

def add_section(final_ds: CVDatasheet, section_title: str, level: int, content: str, place_after: str):
...
def smart_update_section(draft_ds: CVDatasheet, final_ds: CVDatasheet, section_title: str, lang_code: str):
    ...
    if final_section is None:
        ...
        final_section = add_section(final_ds, localized_section, level=3, content=draft_section.content, place_after=
                    get_localize_section(lang_code, SECTION_ORDER.get(section_title, get_localize_section(lang_code, "license"))))

    if section_title == "community_links" and "pontoon" in final_section.content.lower():
...
def remove_placeholder_content(ds: CVDatasheet, section_title: str, lang_code: str):
    ...
    if section_title == "sample":
        placeholder_texts = ["There follows a randomly selected sample of five sentences from the corpus.", "A continuación se muestran cinco oraciones seleccionadas aleatoriamente del corpus."]
        for placeholder in placeholder_texts:
            section.content = section.content.replace(placeholder, "").strip()
        automatic_msg = "*Automatic random samples*" if lang_code == "en" else "*Muestras automáticas aleatorias*"
        section.content = f"\n{automatic_msg}\n\n" + section.content 

def main():
    for modality in MODALITIES:
        for lang_code in LANG_CODES:
            draft_folder = DRAFT_MD_FOLDER.format(modality=modality, release_version=RELEASE_VERSION, release_mode="draft", lang_code=lang_code)
            final_folder = DRAFT_MD_FOLDER.format(modality=modality, release_version=RELEASE_VERSION, release_mode="final", lang_code=lang_code)
            ...
            for md_file in draft_path.glob("*.md"):
                ...
                # append md name to final folder path
                try:
                    md_final_text = read_markdown_file(final_folder + md_file.name)
                except FileNotFoundError:
                    print(f"\t[WARNING]: Final markdown file not found for {md_file.name}. Copying draft to final.")
                    if lang_code == "es":
                    ...
                    new_header_content = f"{datasheet_msg}\n\n {draft_datasheet.header.content}"
                    draft_datasheet.header.content = new_header_content
                    write_markdown_file(final_folder + md_file.name, draft_datasheet.to_markdown(include_empty_sections=True))
                    continue
                final_datasheet = CVDatasheet(markdown_text=md_final_text)
                if modality == "scs":
                    try:
                        replace_content_section(draft_datasheet, final_datasheet, "gender", lang_code)
                    except KeyError:
                        print(f"\t[WARNING]: No gender section found for {md_file.name} Skipping")
                    try:
                        replace_content_section(draft_datasheet, final_datasheet, "age", lang_code)
                    except KeyError:
                        print(f"\t[WARNING]: No age section found for {md_file.name} Skipping")
                    try:
                        remove_placeholder_content(draft_datasheet, "sample", lang_code)                
                        append_content_section(draft_datasheet, final_datasheet, "sample", lang_code)                    
                    except KeyError:
                        print(f"\t[WARNING]: No sample section found for {md_file.name} Skipping")
                    try:
                        smart_update_section(draft_datasheet, final_datasheet, "community_links", lang_code)
                    except KeyError:
                        print(f"\t[WARNING]: No community_links section found for {md_file.name} Skipping")

                    write_markdown_file(final_folder + md_file.name, final_datasheet.to_markdown(include_empty_sections=True))
                elif modality == "sps":
